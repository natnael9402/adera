...
async function runProductMigration() {
  console.log('🚀 Starting 1,000+ Products Migration & Seed Runner (Unique Images Engine)...');
  ...
  for (let i = 0; i < items.length; i += CHUNK_SIZE) {
    ...
    const progress = Math.min(i + CHUNK_SIZE, items.length);
    console.log(`⏳ Processed ${progress} / ${items.length} products...`);
  }

  const totalInDb = await prisma.product.count();
  console.log(`✅ Migration Completed Successfully!`);
  console.log(`📊 Newly Inserted: ${insertedCount} | Updated: ${updatedCount} | Total Products in DB: ${totalInDb}`);
