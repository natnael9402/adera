...
export function generate1000Products(): any[] {
  const products: any[] = [];
  let globalCounter = 1000;

  const categoryBuckets: any[][] = [];
  ...
  // Interleave round-robin across all 12 categories for maximum store diversity
...
export async function runProductMigration(options: { fresh?: boolean } = {}) {
