    ...
    <div className="h-screen bg-[#f8fafc] text-slate-900 flex flex-col font-sans overflow-hidden">
      ...
      <div className="flex-1 flex max-w-[1600px] w-full mx-auto overflow-hidden">
        ...
        <main className="flex-1 h-full overflow-y-auto p-4 sm:p-6 lg:p-8 space-y-6 max-w-full">
          ...
          {/* ========================================================================= */}
          {/* TAB 3: WHOLESALE SOURCING STUDIO (3,927 PRODUCTS)                        */}
          {/* ========================================================================= */}
          {activeTab === 'sourcing' && (
            <div className="space-y-5 animate-fade-in">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {filteredCatalog.slice(0, catalogDisplayLimit).map((product) => {
                  ...
                  return (
                    <div
                      key={product.id}
                      className="bg-white rounded-3xl border border-slate-200 p-4 flex flex-col justify-between space-y-3 shadow-xs hover:border-emerald-300 transition-all group"
                    >
                      <div>
                        <div className="relative aspect-square w-full rounded-2xl overflow-hidden bg-slate-50 mb-3 border border-slate-100">
                          ...
                          {product.isImported && (
                            <span className="absolute top-2 right-2 bg-emerald-600 text-white text-[10px] font-black px-2 py-0.5 rounded shadow-xs flex items-center gap-1">
                        ...
                        <div className="mt-2.5 p-2.5 bg-slate-50 rounded-xl border border-slate-100 flex items-baseline justify-between text-xs">
                          ...
                          <div className="text-right">
                            <span className="text-[9px] text-emerald-700 font-bold uppercase block">Max Resale (+{currentTierMargin}%)</span>
                            <span className="font-mono font-black text-emerald-700">${maxResale.toFixed(2)}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <button
                        ...
                        className={`w-full py-2.5 font-extrabold text-xs rounded-xl transition-all shadow-xs flex items-center justify-center gap-1.5 ${
                          product.isImported
                            ? 'bg-slate-100 hover:bg-emerald-50 text-emerald-700 border border-emerald-200'
                            : 'bg-emerald-600 hover:bg-emerald-500 text-white'
                        }`}
                      >
                        <Plus className="w-3.5 h-3.5" />
                        <span>{product.isImported ? 'Modify Markup Price' : '1-Click Import to Shop'}</span>
                      </button>
                    </div>
                  );
                })}
              ...
              {filteredCatalog.length > catalogDisplayLimit && (
                <div className="pt-4 text-center">
                  <button
                    onClick={() => setCatalogDisplayLimit((prev) => prev + 24)}
                    className="px-8 py-3 bg-slate-900 hover:bg-slate-800 text-white font-extrabold text-xs rounded-xl shadow-md transition-all active:scale-95"
                  >
